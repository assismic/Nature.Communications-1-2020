...
# Pos vcs nown negatives (prometheus)
  ...
  #save.image("SavedSessions/FullAnalysis.Step1.R")
    ...
    for (i in 1:4){ 
      ...
      for (p in 1:ncol(proteinSubsetCombination)){
        ...
        if(row %in% paste(proteinSubsetCombination[,p], collapse = "; ")){
          ...
          #protein.sub.temp.df <- data.frame(iso.sub.temp.df[,proteinSubsetCombination[,p]])
          Classifier <- iso.sub.temp.df$Classifier
          Classifier <- ifelse(test = Classifier == "Positive", yes = 1, no = 0)
          ...
          model <- glm(Classifier ~., family = binomial(link = 'logit'), data = trainingSet)
          #model <- read_rds(paste("c:/temp/Objects/AUCs/.",iso,".", proteinSubsetCombination[,p], ".rds", sep =""))
          ...
          if(auc == ref.auc){
            antigen.title <- row
            mainModel <- model
            #saveRDS(mainModel, file =paste("c:/temp/Objects/AUCs/",iso,".", antigen.title, ".rds", sep = ""))
            print(paste("MATCH", " Round", r, "auc:", auc, "@ digits ", ndigits))
            par(pty ="s")
            #png(paste("c:/temp/Figures/MultiAUC/",iso,".",i,".",row, ".AUC.100.075.tiff", sep=""),width = 960, height = 960,
            ...
            #write.csv(roc.df , paste("c:/temp/Output/rocDFs/",iso,".",i,".",row, ".AUC.csv", sep=""))
            aucList <- c(auc)
            elements <- c("")
            ...
            if(length(singles) > 1){
              ...
              for(s in 1:length(singles)){
                ...
                #saveRDS(submodel, file =paste("c:/temp/Objects/AUCs/",iso,".", antigen.title, ".rds", sep = ""))
                #write.csv(trainIndex.index, paste("c:/temp/Objects/AUCs/",iso,".", antigen.title, "model.trainingIndex.csv", sep = ""))
                #model <-  read_rds(paste("c:/temp/Objects/AUCs/.",iso,".", proteinSubsetCombination[,p], ".rds", sep =""))
                ...
                #write.csv(roc.df , paste("c:/temp/Output/rocDFs/sub.roc.",iso,".",i,".",singles[s], ".AUC.csv", sep=""))
                text(0.3, 0.4, "Combined antigens:", cex = 0.3)
                
              }
            }
            
            
            text(0.15, 0.35, paste("Max. specificity:", specificity), cex = 0.3)
            text(0.15, 0.30, paste("Max. sensitivity:", Sensitivity), cex = 0.3)
            legend("bottomright", c(paste(elements, aucList, sep = " AUC: ")), lwd = 1, col = c("#0361A6", c(colorsBrew[1:length(singles)])), cex = 0.3)
            
            #saveRDS(mainModel, file =paste("c:/temp/Objects/AUCs/",iso, row, "rds", sep = "."))
            #dev.off()
          ...
          while(auc != ref.auc){
            ...
            #write.csv(trainIndex.index, paste("c:/temp/Objects/AUCs/",iso,".", row, "model.trainingIndex.csv", sep = ""))
            ...
            if(auc == ref.auc){
              #saveRDS(model, file =paste("c:/temp/Objects/AUCs/",iso,".", row, ".rds", sep = ""))
              print(paste("MATCH", " Round", r, "auc:", auc, "@ digits ", ndigits))
              par(pty ="s")
              #png(paste("c:/temp/Figures/MultiAUC/",iso,".",i,".",row, ".AUC.100.075.tiff", sep=""),width = 960, height = 960,
              #pointsize = 12, res = 300)
...
for (i in unique(temp.long$Isotype)){
  ...
  for(a in unique(sub.temp.long$Unique.ID)){
    ...
    print(a)
    print(
      ggplot(sub.3.temp.long, aes(x = Infection, y = Norm.MFI, fill=Infection))+#, color = Infection)) +
